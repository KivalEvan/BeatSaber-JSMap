...
export function NJSEventSchema(): v.ObjectSchema<
   InferObjectEntries<INJSEvent>,
   undefined
> {
   return v.object<InferObjectEntries<INJSEvent>>({
      d: field(v.optional(v.number()), {
         version: '4.1.0',
      }),
...
export function DifficultySchema(): v.ObjectSchema<
   ...
   undefined
> {
   return entity<InferObjectEntries<IDifficulty>>((x) => x.version || '4.0.0', {
