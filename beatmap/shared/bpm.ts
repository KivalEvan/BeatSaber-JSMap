        ...
        this._timeScale = this.getTimeScale(
            bpmChange.filter((bc) => {
                ...
                return (bc as IBPMEvent).m != null &&
                    (bc as IBPMChangeV3).o == null;
            }) as IBPMEvent[],
        );
        this._bpmChange = this.getBPMChangeTime(
            bpmChange.filter(
                (bc) =>
                    (bc as IBPMChangeV2)._time != null ||
                    (bc as IBPMChangeV3).o != null,
        ...
        for (let i = 0; i < bpmc.length; i++) {
            ...
            if (temp) {
                curBPMC.newTime = Math.ceil(
                    ((curBPMC.time - temp.time) / this._bpm) * temp.BPM +
                        temp.newTime -
                        0.01,
                );
            } else {
                curBPMC.newTime = Math.ceil(
                    curBPMC.time - (this._offset * this._bpm) / 60 - 0.01,
                );
            }
    ...
    toRealTime(beat: number, timescale = true): number {
        ...
        for (let i = this._timeScale.length - 1; i >= 0; i--) {
            if (beat > this._timeScale[i].time) {
                calculatedBeat += (beat - this._timeScale[i].time) *
                    this._timeScale[i].scale;
    ...
    toBeatTime(seconds: number, timescale = false): number {
        ...
        for (let i = this._timeScale.length - 1; i >= 0; i--) {
            ...
            if (seconds > currentSeconds) {
                calculatedSecond += (seconds - currentSeconds) /
                    this._timeScale[i].scale;
    ...
    toJSONTime(beat: number): number {
        for (let i = this._bpmChange.length - 1; i >= 0; i--) {
            if (beat > this._bpmChange[i].newTime) {
                return (
                    ((beat - this._bpmChange[i].newTime) /
    ...
    adjustTime(beat: number): number {
        for (let i = this._bpmChange.length - 1; i >= 0; i--) {
            if (beat > this._bpmChange[i].time) {
                return (
                    ((beat - this._bpmChange[i].time) / this._bpm) *
                        this._bpmChange[i].BPM +
