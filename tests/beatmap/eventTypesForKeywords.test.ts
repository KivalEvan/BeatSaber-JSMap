...
const classList = [v3.BasicEventTypesForKeywords, v2.SpecialEventsKeywordFiltersKeywords];
const defaultValue = {
    keyword: '',
    events: [],
};
...
Deno.test(`${name} instantiation`, () => {
    ...
    for (const Class of classList) {
        ...
        if (obj instanceof v3.BasicEventTypesForKeywords) obj = new Class({ k: 'test', e: [1, 2] });
...
Deno.test(`${name} to JSON object`, () => {
    for (const Class of classList) {
        ...
        if (obj instanceof v3.BasicEventTypesForKeywords) assertEquals(json, { k: '', e: [] });
        if (obj instanceof v2.SpecialEventsKeywordFiltersKeywords) {
