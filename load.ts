...
function _info(options: ILoadOptionsInfo) {
   ...
   return parseInfo(
      JSON.parse(Deno.readTextFileSync(resolve(opt.directory, opt.filePath))),
   );
}
...
export function info(options: ILoadOptionsInfo = {}): Promise<IInfo> {
   ...
   logger.tInfo(
      tag('info'),
      `Async loading info from ${resolve(opt.directory, opt.filePath)}`,
   );
...
export function infoSync(options: ILoadOptionsInfo = {}): IInfo {
   ...
   logger.tInfo(
      tag('infoSync'),
      `Sync loading info from ${resolve(opt.directory, opt.filePath)}`,
   );
   return _info(opt);
}
...
function _difficulty(
   ...
   const diffJSON = JSON.parse(
      Deno.readTextFileSync(resolve(opt.directory, filePath)),
   ) as Record<
      string,
      unknown
   >;
