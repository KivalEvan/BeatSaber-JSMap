...
function v2(data: DifficultyV2) {
    logger.debug(
        '[patch::removeOutsidePlayable::v2] Removing outside playable notes',
    );
    ...
    logger.debug(
        '[patch::removeOutsidePlayable::v2] Removing outside playable events',
    );
    ...
    data.sliders = data.sliders.filter((
        obj,
    ) => (duration ? !(obj.time < 0 || obj.time > duration) : !(obj.time < 0)));
}
...
function v3(data: DifficultyV3) {
    ...
    if (data.customData.fakeColorNotes) {
        data.customData.fakeColorNotes = data.customData.fakeColorNotes.filter((
            obj,
    ...
    if (data.customData.fakeBombNotes) {
        data.customData.fakeBombNotes = data.customData.fakeBombNotes.filter((
            obj,
        ) => duration ? !(obj.b < 0 || obj.b > duration) : !(obj.b < 0));
    }
    logger.debug(
    ...
    if (data.customData.fakeObstacles) {
        data.customData.fakeObstacles = data.customData.fakeObstacles.filter((
            obj,
        ) => duration ? !(obj.b < 0 || obj.b > duration) : !(obj.b < 0));
    }
    logger.debug(
    ...
    if (data.customData.fakeBurstSliders) {
        data.customData.fakeBurstSliders = data.customData.fakeBurstSliders
    ...
    data.lightColorEventBoxGroups = data.lightColorEventBoxGroups.filter(
        filterTime,
    );
    logger.debug(
